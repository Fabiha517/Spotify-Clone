...
meq.addEventListener("change", () => {
  const playbar = document.querySelector(".playbar");
  if (meq.matches) {
    // If switching to mobile view AND audio is playing, show the playbar
    if (!audio.paused) {
    ...
    // If switching to desktop, always show playbar
    playbar.style.display = "block";
  }
});
...
function playSong(index) {
  ...
  // Show the playbar on mobile(width 500px or less).Only when a song starts playing.

  if (window.matchMedia("(max-width: 500px)").matches) {
    document.querySelector(".playbar").style.display = "block";
  }
}
// Hide the playbar when the song ends — only on mobile
